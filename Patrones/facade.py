...
class ARFacade:
    """
    Clase que sirve como interfaz de la funcionalidad ofrecida por RunAR
    """

    def __init__(self, objectreadfile, makefullscreen, makeoptimize):
        """
        Constructor de la clase ARFacade
        """
        self.hands_detection = hand_detector()
        self.captured = Capture(makeoptimize=makeoptimize)
        self.viewer = ObjectViewer(objectreadfile, makefullscreen=makefullscreen)
...
class RunAR:
    ...
    def __init__(self):
        ...
        self.facade = ARFacade(self.objectreadfile, self.makefullscreen, self.makeoptimize)

    def run(self):
        """
        Método que ejecuta la detección y visualización de objetos en 3D a través de la interfaz de ARFacade
        """
        self.facade.run()